  ...
  return (
    ...
    >
      ...
      <Box direction="Column" gap="100">
        <Text size="L400">{t('token_login.title')}</Text>
        <Text size="T300">
          <b>{message}</b>
        </Text>
      </Box>
      ...
      {loginState.status === AsyncStatus.Error && (
        <>
          {loginState.error.errcode === LoginError.Forbidden && (
            <LoginTokenError message={t('token_login.invalid_token')} />
          )}
          {loginState.error.errcode === LoginError.UserDeactivated && (
            <LoginTokenError message={t('token_login.account_deactivated')} />
          )}
          {loginState.error.errcode === LoginError.InvalidRequest && (
            <LoginTokenError message={t('token_login.invalid_request')} />
          )}
          {loginState.error.errcode === LoginError.RateLimited && (
            <LoginTokenError message={t('token_login.rate_limited')} />
          )}
          {loginState.error.errcode === LoginError.Unknown && (
            <LoginTokenError message={t('token_login.unknown_error')} />
          )}
        </>
      )}
