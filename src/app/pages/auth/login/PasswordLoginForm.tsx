        ...
        >
          <Menu>
            <Header size="300" style={{ padding: `0 ${config.space.S200}` }}>
              <Text size="L400">{t('login.hint')}</Text>
            </Header>
            <Box
        ...
        {loginState.status === AsyncStatus.Error && (
          <>
            {loginState.error.errcode === LoginError.ServerNotAllowed && (
              <FieldError message={t('login.custom_server_not_allowed')} />
            )}
            {loginState.error.errcode === LoginError.InvalidServer && (
              <FieldError message={t('login.failed_find_matrix_id')} />
            )}
          </>
        )}
      </Box>
          ...
          {loginState.status === AsyncStatus.Error && (
            <>
              {loginState.error.errcode === LoginError.Forbidden && (
                <FieldError message={t('login.invalid_credentials')} />
              )}
              {loginState.error.errcode === LoginError.UserDeactivated && (
                <FieldError message={t('login.account_deactivated')} />
              )}
              {loginState.error.errcode === LoginError.InvalidRequest && (
                <FieldError message={t('login.invalid_request')} />
              )}
              {loginState.error.errcode === LoginError.RateLimited && (
                <FieldError message={t('login.rate_limited')} />
              )}
              {loginState.error.errcode === LoginError.Unknown && (
                <FieldError message={t('login.unknown_error')} />
              )}
            </>
          )}
